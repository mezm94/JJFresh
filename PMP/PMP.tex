...
\chapter*{Executive Summary}\label{sec:ESummary}
   Jess and James plans to create an online store of fruits and vegetables as a JJFresh online sales platform for users to select and place orders. In this way, they want to maintain their existing customer base and remain competitive with other stores. Since Jess and James have always been small businesses, the budget is not sufficient, the students of registered software project management of the University of Melbourne are employed to develop this system.    
   Firstly, the development team fully understood the needs of Jess and James. After two group meetings, they determined all the requirements, development models and risk analysis of the project. In addition, predict the possible problems and put forward corresponding solutions.Based on the consideration of the changes in the needs of the project itself and the size of the development team, they chose an agile Scrum model, which will be assigned tasks to members by a short Sprint. In the first phase (4.28-5.8), they plan to deliver a product that meets core requirements to Jess and James within 2 weeks. For instance, the website can implement basic functions such as user login, product selection, and order cancellation. After discussing with Jess and James, it is expected that the second stage of development work will begin on May 11. The core work at this stage is to improve other user needs, such as presenting JJFresh products in a more beautiful way. In the end, 3 iterations of development will be completed within 5 weeks (4.27-5.29), and then the complete website will be delivered.
   The entire development cycle is expected to be 5 weeks, and it will cost Jess and James about $ 3,000 to complete.The product will provide Jess and James with an online sales channel, thereby bringing them additional economic benefits. On the other hand, for the development team, the project will accumulate actual development experience for school students, which will improve their competitiveness in future job search.
...
\begin{tabularx}{0.95\linewidth}{%
  ...
  & 70%
  & 4
  & After the development team chose wix, although wix itself can quickly implement some basic functions, it may not be convenient to implement the extended functions of Jess and James later. For example, when they later want buyers to pay online, they may not be able to use Wix directly. Failure to meet all user needs will result in the failure of the project's later expansion functions.
  \\
  \midrule
  2
  & Business
...
\begin{tabularx}{0.95\linewidth}{%
  ...
  & The development team can add .html files or JavaScript technology when using wix. Although it takes more time for the development team, it can achieve the user's expansion needs. This risk can also be controlled.
  & Mitigate or avoid 
  & The development team needs to design and add .html files or use JavaScript technology.
  \\
  \midrule
  2
  & Jess or James is sick or the coronavirus is serious
  & Jess and James
  ...
  & Mitigate or accept
  & Jess and James needs to spend more cost when handling the delivery
  \\
  \midrule
  3
  ...
  & Mitigate
  & Jess and James need to focus on delivery orders at the same address, and determine whether they are reasonable orders.
  \\
  \midrule
  4
  & User cannot receive mail
  & users(Jess & James and users)
...
There are two developers in our team. Since both of them are students, each of them can only spend about 1.2 to 1.5 hours a day on the project. They can work five days a week, so the total working hours of a week are 12 to 15 hours. We expect each story point to be equivalent to one hour of working time. So the min velocity ($V_{min}$) for a two-week sprint is about 24 story points, and the max velocity ($V_{max}$) is about 30 story points. And by using the formula shown below, we can get our minimum and maximum number of sprints, which are 2 and 2.5, respectively. The inital burndown chart is shown on figure \ref{fig:burndown1}.
$$
...
\caption{Initial Burndown Chart}
\label{fig:burndown1}
\end{figure}
